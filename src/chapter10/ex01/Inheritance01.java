...
class Student extends Human{
	//부모 클래스 : Human, 자식 클래스 : Student <==부모클래스의 필드와 메소드를 물려 받는다. //자식에서 부모로 상속관계이다.
	
	//1.부모 클래스의 모든 필드와 메소드를 그대로 자식 클래스 내부에 들어와있다.
	//2.자식 클래스에서만 적용되는 필드와 메소드를 추가적으로 등록해서 사용
	int studentID;
	void goToSchool() {
		System.out.println("학생은 학교에 다닌다.");
	}
}
...
class Professor extends Human{
	int professorID;
	void goToTeacher() {
		System.out.println("교수님은가르치러 갑니다.");
	}
}
...
public class Inheritance01 {
	...
	public static void main(String[] args) {
		// 상속 (Inheritance) : 부모 클래스의 필드와, 메소드를 자식 클래스에서 물려 받아서 사용//부모클래스에는 필드,생성자(생성자는 상속 안됨), 메소드가 있다.
			// 객체 지향 언어의 꽃(제일 중요함),
			// 중복된 코드를 줄여 줄 수 있다.
			// 다향성 : 부모클래스를 상속 받아서 다양한 형태로 출력
			// 배열이나 컬렉션에 부모클래스 이름으로 (업캐스팅) 저장 해서 출력
		...
		System.out.println("=========worker Human 클래스 객체 생성==============");
