...
void animalSound()
	{
		System.out.println("독수리는 무야호 하고 웁니다.");
	}
}
...
public class Method_Overriding02 {
	...
	public static void main(String[] args) {
		...
		t2.animalSound();	//Animal의 animalSound 호출시, Lion의 animalSound가 출력
		t3.animalSound();	//Animal의 animalSound 호출시, Eagle의 animalSound가 출력
		
		
		System.out.println("===========================");
		
		//배열에 객체를 저장
		//arr1에 Animal 객체가 저장된다.
		Animal[] arr1 = new Animal[3];
		
		//1.arr1 배열의 각방에 Animal 객체 저장
		arr1[0] = t1;
		arr1[1] = t2;
		arr1[2] = t3;
		
		//2. arr1 배열에 Animal 타입에 객체가 저장이 되어 있음.
				//배열의 각 방의 값을 가지고 오기
		Animal a1 = arr1[0];	//Animal타입으로 변수 선언후 aar1에 있는 배열을 Animal 출력?
		Animal a2 = arr1[1];	
		Animal a3 = arr1[2];
