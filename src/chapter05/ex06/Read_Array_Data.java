...
public class Read_Array_Data {
	...
	public static void main(String[] args) {
		// arr1.length : - 배열 방의 개수를 출력
			// 1차원 배열 일때 - 배열 방의 열의 개수를 출력
			// 2차원 배열 일때 - 배열 방의 행의 개수를 출력
				// arr1[0].length : 0번행의 열의 갯수를 출력
		//1. 1차원 배열일때 :  arr1.length
		int[] arr1 = new int[300];
		System.out.println(arr1.length); //방의 개수 (열의 개수)
		...
		for ( int i =0; i < arr3.length; i++ ) //i : 행의 방번호 , arr2.length: 행의 개수 // 바깥쪽 for문은 행의 정보를 입력
		{
			for (int j = 0; j <arr3[i].length; j++)//j:열의 방번호  arr2[i].length : 열의 개수 //열의 정보를 출력
			{
