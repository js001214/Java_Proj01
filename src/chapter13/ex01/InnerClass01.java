...
/*
  ...
  1. 인스턴스 이너 클래스 : static키가 붙지 않는 클래스
...
class A { //Outer class : 필드, 메소드에 접근제어자가 붙어있다.
			//Inner class를 생성하면 Outer class의 접근 제어자 상관 없이 접근
	
	public int a= 3;		//다른 패키지에서 접근 가능, 같은 패키지에서도 접근 가능
	protected int b = 4;	//다른 패키지에서 접근가능(상속), 같은 패지키에서도 접근가능
	int c = 5;				//default , 다른 패키지에서는 접근 불가능, 같은 패키지에서만 접근가능
	private int d = 6;		//같은 클래스에서만만 접근 가능
	...
	// 이너클래스 : 인스턴스 이너클래스, 1. 아웃터 클래스의 접근제어자와 상관없이 접근, 
...
public class InnerClass01 {
	...
	public static void main(String[] args) {
		...
		//2. 이너클래스의 객체 생성
		A.B b  = a.new B();	//A.B : 외부클래스.내부클래스 :  A.B는 내부클래스의 타입
							//b는 참조변수
							//a.new B();	<==외부인스턴스명.new B() 내부 클래스 생성자;
