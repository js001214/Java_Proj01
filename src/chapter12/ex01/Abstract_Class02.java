...
//추상 메소드를 구현하는 방법 2가지
	//1. 자식클래스에서 구현한 클래스를 생성하고 출력 : 매번 자주 사용해야 되는 경우 
			//출력코드가 간결 하다. 구현한 클래스를 생성해야 한다.
	//2. 익명클래스를 생성해서 출력 : 한 번 사용하고 버릴 경우, 안드로이드 폰 개발 할때, 이벤트 처리 할때 사용.
			//추상클래스의 구현한 자식 클래스 생성없이 바로 사용이 가능
			//여러번 사용할때는 구현한 코드를 매번 생성해야 한다. 

//1.첫 번째 방법 : 추상 클래스(A) <== 구현한 클래스(B)
abstract class A{	//추상 클래스
	abstract void cry();
}


//2.두 번째 방법 : 추상 클래스를 구현한 클래스 없이 바로 출력 : 익명 클래스를 사용해서 
abstract class C{
	abstract void run();
}
//구현 클래스를 생성 : <<

class B extends A{		//1. 자식 클래스에서 추상클래스의 메소드를 구현한 클래스를 생성ㅌ
	@Override
	void cry() {
...
public class Abstract_Class02 {
	...
	//	A a = new A();				//추상 클래스는 객체 생성 불가능//보통은 추상클래스를 구현한 자식 클래스를 이용
